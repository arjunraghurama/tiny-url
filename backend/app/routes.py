...
async def shorten_url(request: ShortenRequest, db: AsyncSession = Depends(get_db)):
    ...
    The short code is generated using Base62 encoding of the database ID.
    """
    url_record = await create_short_url(db, str(request.url))
...
async def list_recent_urls(db: AsyncSession = Depends(get_db)):
    ...
    Returns the 10 most recent URLs with click stats.
    """
    urls = await get_recent_urls(db)
    return [
        URLListItem(
